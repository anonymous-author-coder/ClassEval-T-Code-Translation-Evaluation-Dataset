...
public class Snake {
    ...
    public void move(Direction direction) {
        ...
        Point newPoint = new Point(
            ((cur.getX() + (x * BLOCK_SIZE)) % SCREEN_WIDTH),
            ((cur.getY() + (y * BLOCK_SIZE)) % SCREEN_HEIGHT)
        );
    ...
    public void randomFoodPosition() {
        while (positions.contains(foodPosition)) {
            foodPosition = new Point(
                new Random().nextInt((SCREEN_WIDTH / BLOCK_SIZE) - 1) * BLOCK_SIZE,
                new Random().nextInt((SCREEN_HEIGHT / BLOCK_SIZE) - 1) * BLOCK_SIZE
            );
        }
    }
