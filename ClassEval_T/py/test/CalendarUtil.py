...
class CalendarTestGetUpcomingEvents(unittest.TestCase):
    ...
    def test_get_upcoming_events_5(self):
        calendar = CalendarUtil()
        now = datetime.now()
        calendar.events = [{'date': now-timedelta(minutes=6*30*24*60), 'start_time': now-timedelta(minutes=6*30*24*60),
                            'end_time': now-timedelta(minutes=6*30*24*60)+timedelta(minutes=23*60), 'description': 'New Year'},
                           {'date': now+timedelta(minutes=6*30*24*60), 'start_time': now+timedelta(minutes=6*30*24*60)+timedelta(minutes=60),
                            'end_time': now+timedelta(minutes=6*30*24*60)+timedelta(minutes=120),
                            'description': 'New Year 2'}]
        self.assertEqual(calendar.get_upcoming_events(1), [
            {'date': now + timedelta(minutes=6 * 30 * 24 * 60),
             'start_time': now + timedelta(minutes=6 * 30 * 24 * 60) + timedelta(minutes=60),
             'end_time': now + timedelta(minutes=6 * 30 * 24 * 60) + timedelta(minutes=120),
             'description': 'New Year 2'}])
